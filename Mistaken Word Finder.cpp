...
int main()
{
   ...
   char aux[50];
    ...
    if(c == 'E' || c == 'e')
    {
        ...
        if(ok)
        {
          ...
          while(answer=='y')
          {
            ...
            while(x != 1)
            {
                ...
                cout << "    >>> Enter the file name ( Format : file.txt ) : "; cin.get(file, 20);cin.get();
           ...
           cout << '\n' << '\n' << '\n' << "   >>>  File (" << nr_file <<") succesfully processed ! Here is your text : " << '\n' << '\n' << '\n';
           cout << "       ";
           ...
           /// Prelucrare text
            ...
            /// Succes
            cout << '\n' << '\n';
            cout << "   ";SetConsoleTextAttribute(hConsole, 233);
            ...
            cout << '\n' << '\n';
            cout << "   >>> Press 'n' and then ENTER to log out"; cin >> answer; cout << '\n' << '\n';
            ...
            if(unload_dictionary(hashtable))  cout << "       -> Application succesful ... No memory leaks detected" << '\n' << '\n';
            else cout << "       -> Application error ... Memory leaks detected" << '\n' << '\n';

            char exit;
            cout << '\n' << '\n' << '\n' <<"   >>> Press 'A' and then ENTER to exit " ; cin >> exit;
        ...
        out.close(); load.close(); f.close();


        char exit1;
        cout << '\n' << '\n' << '\n' <<"   >>> Press 'A' and then ENTER to exit " ; cin >> exit1;
